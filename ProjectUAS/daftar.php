...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $jadwal_id = null;
    // PERBAIKAN: Status awal SELALU 'Menunggu Verifikasi' untuk pendaftar baru.
    $status_awal = 'Menunggu Verifikasi'; 
    $kategori = $_POST['kategori'] ?? 'umum';
    
    $waktu_sekarang_utc = (new DateTime('now', new DateTimeZone('UTC')))->format(DateTime::ATOM);
    $encoded_waktu = urlencode($waktu_sekarang_utc);
    
    $endpoint_jadwal = "/rest/v1/jadwal_seleksi?kategori=eq.$kategori&tanggal=gte.$encoded_waktu&order=tanggal.asc&limit=1";
    $jadwal_result = supabase_request("GET", $endpoint_jadwal);

    // Logika ini sekarang HANYA menetapkan jadwal_id, tidak mengubah status.
    if ($jadwal_result['data'] && count($jadwal_result['data']) > 0) {
        $jadwal_id = $jadwal_result['data'][0]['id'];
        // Baris '$status_awal = 'Lulus Administrasi';' telah dihapus dari sini.
    }
    ...
    $data_to_insert = [
        ...
        'foto_url' => $foto_url,
        'status' => $status_awal, // <- Ini sekarang akan selalu 'Menunggu Verifikasi'
        'jadwal_id' => $jadwal_id,
    ];
...
<div class="content-wrapper">
    ...
    <form method="post" enctype="multipart/form-data" action="daftar.php">
        ...
        <img id="profilePicturePreview" src="assets/img/default-avatar.png" alt="Preview Foto Profil" style="width: 150px; height: 150px; border-radius: 50%; object-fit: cover; margin: 10px auto; display: block; border: 3px solid #eee;">
